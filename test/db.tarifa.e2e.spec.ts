...
defineFeature( feature, test => {
  ...
  test( "Tarifas encontradas", ( {
    ...
    then( "recebo uma lista de tarifas", () => {
      expect( resposta.status ).toBe( 302 );
    } );
  } );


  test( "Tarifas não encontradas", ( {
    given,
    when,
    then
  } ) => {
    ...
    given( "O banco de dados está vazio", async () => {
      ...
      AgenciaService.prototype.getTarifas = jest.fn().mockImplementationOnce( () => {
        return [];
      } );
    } );
