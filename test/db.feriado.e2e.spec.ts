...
const feature = loadFeature( "./test/features/db.feriado.feature" );
...
defineFeature( feature, test => {
  ...
  test( "Feriados encontradas", ( {
    given,
    when,
    then
  } ) => {
    given( "quero ver a lista de feriados", async () => {
      endpoint = `${raiz}/agencias/feriados`;
    } );
    ...
    then( "recebo uma lista de feriados", () => {
      expect( resposta.status ).toBe( 302 );
    } );
  } );


  test( "Feriados não encontradas", ( {
    given,
    when,
    then
  } ) => {
    ...
    given( "O banco de dados está vazio", async () => {
      ...
      AgenciaService.prototype.getFeriados = jest.fn().mockImplementationOnce( () => {
        return [];
      } );
    } );
